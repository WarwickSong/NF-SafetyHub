...
> 本文档对 SafetyHub 的全部功能进行系统性拆分与定义，明确每个功能的输入、输出、行为约束和验收标准。当前执行口径为：阶段 7 及之后暂不开发，近期聚焦阶段 6A 单实例 Docker 生产稳定性与高并发治理，确保生产上线稳定；阶段 7~10 相关功能仅作为长期规划保留。
...
| F7-C4 | 阶段 8 启用导出功能后支持时间范围筛选，单次导出上限 10000 条 |
