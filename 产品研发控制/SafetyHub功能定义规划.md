...
> 本文档对 SafetyHub 的全部功能进行系统性拆分与定义，明确每个功能的输入、输出、行为约束和验收标准。当前执行口径为：阶段 7 及之后暂不开发，近期只完善阶段 6 及之前能力，确保生产上线稳定；阶段 7~10 相关功能仅作为长期规划保留。
...
| **功能 ID** | F3 |
| **优先级** | P0 — 必须实现 |
| **描述** | Chat 对话的 prompt 和 response 完整记录到 SQLite，支持按用户/时间/关键词检索；阶段 3 同步记录文生图元数据，并异步保存文生图图片本体和归档状态 |
