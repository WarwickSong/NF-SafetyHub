...
> 本文档对 SafetyHub 的全部功能进行系统性拆分与定义，明确每个功能的输入、输出、行为约束和验收标准。当前执行口径为：阶段 6A 后新增阶段 6B 报表中心一期，交付日报、周报、月报与 PDF/XLSX/CSV 下载；阶段 7~10 相关功能仍作为长期规划保留。
...
4. **核心归档** — Chat 对话的 prompt 和 response 完整归档；文生图同步归档元数据，并异步归档图片本体和下载/解码状态
5. **审计追溯** — 拦截事件独立记录，支持多维查询
6. **实时告警** — 阶段 8 长期规划能力，当前暂不开发，不作为生产上线阻塞项
7. **治理预留** — 为 APIKey 映射、安全策略、多上游路由、临时审批提前预留数据结构和接口边界；模型权限、token 额度和资源能力权限由中转站管理
8. **文件安全** — 阶段 10 长期规划能力，当前暂不开发，不作为生产上线阻塞项
9. **生产稳定性治理** — 阶段 6A 当前生产上线范围，在单实例 Docker 前提下提供 `/v1/*` 有界并发队列、管理端保护、上游连接池复用和审计归档削峰
10. **数据治理** — 阶段 6A 已落地训练数据沉淀、覆盖分析、清理预览和手动清理；覆盖分析按 `user_id + api_key_id` 分组寻找最长有效轨迹集合，部署升级保留 `api_keys` 表、不重新导入 APIKey
11. **报表中心** — 阶段 6B 新增能力，支持日报、周报、月报定时生成、手动覆盖重生和 PDF/XLSX/CSV 下载
...
| F1-C8 | 统一生成或透传 `X-Request-ID`，并写入训练样本、审计、图片资产、告警和结构化日志 |
...
| F1-C13 | 阶段 6A 对所有 `/v1/*` 请求统一进入全局有界并发队列，结合 100 名员工同时使用 OpenClaw/Hermes/批量标注 Agent 的峰值生产场景，默认 4 worker 下容器总目标至少 `1000` in-flight + `2000` 排队，队列满或排队超时返回明确错误，不允许无限排队 |
| F1-C14 | 阶段 6A `/admin/*`、`/admin/api/*`、`/health/*` 不进入 `/v1/*` 队列，保证压测期间管理端和健康检查可用 |
| F1-C15 | 阶段 6A 上游转发复用应用级 HTTP 连接池，不允许每个请求重复创建上游 client |
...
- [ ] 阶段 6A 高并发下 `/v1/*` 在途请求数、排队数和超时行为符合配置，默认 4 worker 下覆盖容器总 `1000` in-flight + `2000` 排队目标
- [ ] 阶段 6A 压测期间 `/health/*` 和 `/admin/*` 可访问
...
### F3 — 训练样本与轻量审计存储（Training Samples & Lightweight Evidence）

| 项目 | 说明 |
|------|------|
| **功能 ID** | F3 |
| **优先级** | P0 — 必须实现 |
| **描述** | 为降低长期运行空间占用，当前运行链路不再写入完整 `message_archives` 归档表。通过安全检查的 Chat 对话沉淀到 `training_conversations`，命中/脱敏/拦截证据写入 `audit_logs`，文生图图片资产索引写入 `image_assets`；管理员“训练样本”和“上线观测”从训练样本表读取。|

**输入**

| 输入项 | 类型 | 说明 |
|--------|------|------|
| Chat 训练样本 | ArchivePayload → TrainingConversation | request_id、user_id、api_key_id、model、capability、messages、assistant_response、trajectory、trajectory_hash、脱敏状态、过期时间 |
| 审计证据 | AuditPayload / AuditLog | request_id、规则 ID、规则名称、级别、scanner_type、命中片段、上下文片段、脱敏片段、全文 hash、动作 |
| 图片资产 | ImageAsset | request_id、source_type、source_url、local_path、sha256、mime_type、size_bytes、status、error、created_at、completed_at |

**当前存储模型**

| 表 | 长期定位 | 说明 |
|----|----------|------|
| `training_conversations` | 训练样本主表 | 仅保存通过安全检查且可提取 assistant response 的 Chat 样本，用于训练样本页、上线观测、覆盖分析和清理 |
| `audit_logs` | 安全审计主表 | 保存命中/脱敏/拦截证据，不保存完整 prompt/response，支持追溯和过期清理 |
| `image_assets` | 图片资产索引 | 保存文生图 URL/b64 图片本体归档后的本地路径、hash、大小、状态和错误 |
| `message_archives` | 已移除旧表 | 当前模型、运行链路、管理员页面和部署重建流程均不再保留该完整归档表 |

**查询接口**

| 接口 | 方法 | 说明 |
|------|------|------|
| `/admin/api/archives` | GET | 兼容路径，当前分页查询 `training_conversations` 训练样本，支持 user_id / model / keyword / time_range 筛选 |
| `/admin/api/archives/{id}` | GET | 兼容路径，当前查看训练样本详情，返回 messages、assistant response 和基础元信息 |
| `/admin/api/archives/stats` | GET | 兼容路径，当前统计训练样本总量、脱敏数、模型分布，blocked 固定由审计页承担 |
| `/admin/api/observations/recent` | GET | 查询最近少量训练样本，用于上线观测 role/messages/response 结构 |
| `/admin/api/audits` | GET | 查询命中/脱敏/拦截审计证据 |
| `/admin/api/image-assets` | GET | 查询图片资产索引 |

**行为约束**

| 约束 ID | 约束内容 |
|---------|---------|
| F3-C1 | 训练样本、审计证据、图片资产写入必须异步或轻量调度，不能阻塞请求转发主链路 |
| F3-C2 | Chat 通过请求只写入 `training_conversations`，不再写入完整 `message_archives` |
| F3-C3 | block / warn / desensitize 证据写入 `audit_logs`，不依赖完整归档表追溯 |
| F3-C4 | 文生图只写 `image_assets` 图片资产索引和本体归档结果，不再写图片 metadata 到 `message_archives` |
| F3-C5 | 训练样本页只展示 passed Chat 样本；被拦截、脱敏、warn 的安全证据统一到拦截审计页查看 |
| F3-C6 | `message_archives` 旧完整归档表已从当前 SQLAlchemy 模型和运行链路移除，部署重建后不再创建该表 |
| F3-C7 | `training_conversations`、`audit_logs`、`image_assets` 必须配合数据治理和保留策略，避免长期无限增长 |

**验收标准**

- [x] Chat passed 请求写入 `training_conversations`，形成 messages + assistant response 的 trajectory
- [x] Chat block / desensitize / warn 证据写入 `audit_logs`
- [x] 文生图 URL / b64_json 图片本体异步归档并写入 `image_assets`
- [x] `/admin/api/archives` 和 `/admin/api/observations/recent` 从训练样本表读取
- [x] `message_archives` 不再由运行链路写入
- [x] 训练样本、审计、图片资产写入失败不影响请求主链路
- [ ] 训练样本分页查询在 10 万条数据时 < 200ms
...
### F6.1 — 弱扫描 MVP 与渐进式上线（阶段 2 / 阶段 7 暂停）

| 项目 | 说明 |
|------|------|
| **功能 ID** | F6.1 |
| **优先级** | P0 — 阶段 2 必须实现 |
| **描述** | 阶段 2 上线时仅启用极保守的扫描规则集（手机号脱敏 + 3-5 条几乎不会触发的关键词），完整的扫描链路（normalizer + scanner + 脱敏 + 伪装回复）已经就绪；阶段 7 完整规则集升级暂不开发，当前生产上线继续沿用阶段 2 低干扰规则集 |
...
阶段 7 及之后暂不开发，以下内容仅作为长期规划保留，不纳入当前生产上线范围。
...
| F6.1-C5 | 脱敏后的 prompt 是发往中转站的实际内容；阶段 6A 起完整归档表已弃用，脱敏/命中证据以 `audit_logs` 轻量留存，passed 样本以 `training_conversations` 留存 |
| F6.1-C6 | 阶段 6A 起 desensitize/warn/block 命中事件写入 `audit_logs`；passed Chat 进入 `training_conversations`，不写完整归档表 |
...
- [x] passed Chat 写入 training_conversations；完整 message_archives 归档写入已弃用
...
|------|------|------|
| id | INTEGER PK | 自增主键 |
| request_id | TEXT FK | 关联请求 ID，可与训练样本或图片资产按 request_id 关联 |
| user_id | TEXT | 用户标识 |
...
| `/admin/api/reports/*` | GET/POST | 周期报表生成与下载，阶段 6B 启用；不再为 F7 单独定义审计导出接口 |
...
| F7-C4 | 审计明细导出由阶段 6B 报表中心按周期承接，默认不导出完整上下文 |
...
- [ ] 审计记录无法被修改或删除
- [ ] 阶段 6B 启用后，报表中心 CSV 明细可在 Excel 中打开
...
| 训练样本 | `/admin/archives` | 通过请求样本列表，支持搜索和查看详情 |
...
| 阶段 6A 并发治理 | 对 `/v1/*` 设置最大在途、最大排队和排队超时，默认目标至少为容器总 `1000` in-flight + `2000` 排队，保护单实例主链路 |
| 阶段 6A 管理端保护 | `/admin/*`、`/admin/api/*`、`/health/*` 不进入 `/v1/*` 队列，后台统计可短缓存 |
| 阶段 6A 连接池与削峰 | 当前已通过共享 `httpx.AsyncClient` 复用上游连接池，审计归档进入 `ArchiveQueue` 有界后台队列并支持降级 |
| 阶段 6A 运行状态 | 当前已提供 `/admin/api/runtime`，返回 worker、`/v1` 并发、归档队列、上游连接池和后台缓存配置快照 |
| 指标与日志 | Prometheus 指标暂不开发；当前以健康检查、运行状态 API、响应头和结构化日志摘要支撑上线验证，禁止 prompt/response 原文进入运行日志 |
| 数据保留 | 支持归档 TTL、审计 TTL、清理任务和备份恢复策略 |
| 图片资产治理 | 阶段 3 已支持文生图图片本体异步归档和状态记录；阶段 8 增强后台预览/下载页面、存储配额、保留天数和清理任务 |
...
| F14-C8 | `/v1/*` 并发治理必须使用有界队列，禁止无限排队或无限创建任务；默认 4 worker 下每 worker `250` in-flight + `500` 排队，并支持通过 `.env` 或等价配置调整 |
| F14-C9 | 管理端和健康检查不得被 `/v1/*` 并发闸门拒绝，压测期间必须保留运维入口 |
| F14-C10 | 高并发压测期间日志不得输出完整 prompt/response，避免 Docker stdout 成为瓶颈和泄密点 |
| F14-C11 | SQLite 仅作为兼容和回退参考；当前上线运行库已切换 PostgreSQL，生产高并发需验证 PostgreSQL 连接稳定性、备份恢复和写入削峰策略 |
...
| **P2：APIKey 数据表** | `storage/models.py` | 新建 `api_keys` 表（含 `key_hash`、`key_prefix`、`provider_name`、`upstream_key_id`、`upstream_key_encrypted`、`safetyhub_key_encrypted`、`status`、`expires_at` 等字段，不包含 `model_allowlist`、`capability_allowlist`、配额或速率限制字段），通过 `create_all` 一并建表 | 阶段 5/6 由 `ApiKeyService` 写入和查询，作为 SafetyHub Key ↔ 中转站 Key 的映射表，不作为中转站资源权限源 |
...
| `oneapi_nanfu_yxai` | 当前南孚 yxai OneAPI/OneHub 中转站 | 支持，调用 `/api/token` 并通过 `/api/token/{id}/key` 获取完整 Key | 支持，调用 `DELETE /api/token/{id}` | 已实现；需配置 `KEY_PROVIDER_BASE_URL`、`KEY_PROVIDER_USERNAME`、`KEY_PROVIDER_PASSWORD_ENV`、`KEY_PROVIDER_AUTH_VERSION` |
| `openai_compat` | 兼容 OpenAI Key 管理协议的网关 | 支持 | 支持 | 通过 `/v1/api-keys` 接口，待后续实现 |
...
- Provider 创建默认 K-Sync，创建成功响应会返回完整 SafetyHub Key 供管理员复制；列表接口默认只返回前后缀
- 前端不直接调用中转站，只调用 SafetyHub 自身接口；当前页面允许选择“手动录入”或“由 KeyProvider 创建”
- 管理员后续可通过受审计的 reveal 接口按需显示/复制完整 SafetyHub Key；该接口设置 `Cache-Control: no-store`
...
| F15.2-C4 | 中转站真实 Key 必须加密存储（`upstream_key_encrypted` 字段），客户端 SafetyHub Key 也加密存储（`safetyhub_key_encrypted` 字段），使用 `SAFETYHUB_DATA_KEY` 解密；列表和日志只展示前后缀，完整 Key 仅通过受审计 reveal 接口按需返回 |
| F15.2-C5 | `KeyProvider` 接口签名一旦定型，新增能力（如配额查询）必须通过可选方法或扩展接口加入，不得破坏既有 Provider 实现 |

**验收标准**

- [x] 阶段 1~3：`upstream_api_key=None` 走透传路径，所有原有测试通过
- [x] 阶段 3：`api_keys` 表自动创建，Schema 覆盖阶段 5/6 需要字段
- [x] 阶段 3：`key_provider_type` 配置存在但不影响运行
- [x] 阶段 5：能在管理后台手动录入历史中转站 Key，默认 K-Sync 使用
- [x] 阶段 6：能在管理后台通过 Provider 一次性创建 SafetyHub Key 和中转站 Key
- [ ] 阶段 6 迁移增强：在 `oneapi_nanfu_yxai` 和 `static` 之间切换演练，前端代码和核心链路代码均无改动
- [ ] 阶段 6 迁移增强：路径 C 自动续约迁移支持进度展示、中断恢复和失败重试
- [x] 阶段 6：吊销 Provider Key 时同时吊销中转站对应 Key，并写入管理员操作审计
- [x] 阶段 6：完整 SafetyHub Key 支持受审计 reveal/复制，列表默认不返回明文
- [x] 阶段 6：现有 yxai Key 可通过 `scripts/import_yxai_keys.py` 幂等导入 SafetyHub 数据库
...
| `training_conversations` 治理字段 | `storage/models.py` | `covered_by_conversation_id`、`analysis_status` 等字段支持覆盖分析和清理 |
...
### F21 — 报表中心（Report Center）

| 项目 | 说明 |
|------|------|
| **功能 ID** | F21 |
| **优先级** | P0 — 阶段 6B 一期实现 |
| **描述** | 面向超级管理员生成、归档和下载 SafetyHub 日报、周报、月报，输出 PDF、XLSX 和 CSV 文件，用于管理汇报、留档和安全运营分析 |

**权威规划**

详见 `SafetyHub报表中心一期规划.md`。该文档是报表周期、生成策略、文件格式、数据口径、接口、数据模型和验收标准的权威来源。

**角色与权限**

| 角色 | 可执行动作 | 限制 |
|------|------------|------|
| 超级管理员 | 查看报表列表、手动覆盖重生、选择敏感字段、下载 PDF/XLSX/CSV | 仅全局报表，一期不做部门级或普通用户自助报表 |
| 定时任务 | 自动生成日报、周报、月报，清理过期文件和元数据 | 定时生成默认不包含敏感明细字段 |

**业务规则**

| 约束 ID | 约束内容 |
|---------|----------|
| F21-C1 | 日报按自然日，周报按周一到周日，月报按自然月，统一 `Asia/Shanghai` |
| F21-C2 | 自动生成时间错峰：日报 02:00、周报 03:00、月报 04:00 |
| F21-C3 | 报表文件保存到既有 `/app/data/reports` 挂载目录，数据库只保存元数据和相对路径 |
| F21-C4 | 报表文件和元数据均保留 3 个月，过期后可清理，必要时可重新生成 |
| F21-C5 | 同报表类型、同周期重复生成采用覆盖策略，不保留历史版本 |
| F21-C6 | 同报表类型、同周期必须有任务锁或唯一约束，避免定时任务和手动重生并发覆盖 |
| F21-C7 | 定时生成默认不导出命中片段、脱敏片段和上下文片段 |
| F21-C8 | 手动生成可选择敏感字段，包含敏感字段时元数据必须标记 `include_sensitive=true` |
| F21-C9 | 下载接口必须鉴权，不直接暴露报表静态目录 |
| F21-C10 | 生成失败必须记录失败状态和错误摘要，并复用既有 Webhook 通道告警 |
| F21-C11 | 报表中的系统运行状态必须来自周期采样数据，不使用生成时刻快照替代日报、周报或月报周期状态 |
| F21-C12 | 运行状态采样默认 5 分钟一次，采样数据与报表文件、报表元数据同样保留 3 个月 |
| F21-C13 | PDF 中系统运行状态以曲线展示为主，数字表格仅作为峰值、均值、异常次数等辅助摘要 |

**验收标准**

- [ ] 报表中心可查询日报、周报、月报列表和状态
- [ ] 管理员可手动覆盖重生指定周期报表
- [ ] 定时任务可错峰生成上一完整周期报表
- [ ] 成功报表可下载 PDF、XLSX 和 CSV
- [ ] 敏感字段导出规则符合定时默认关闭、手动显式选择的口径
- [ ] 报表以曲线展示周期内运行状态采样聚合结果
- [ ] 报表目录、备份恢复和离线部署说明同步更新
...
| 中继转发 | F1 | P0 | 阶段 1 |
| 伪装回复 | F2 | P0 | 阶段 2 |
| 训练样本与轻量审计存储 | F3 | P0 | 阶段 3 / 阶段 6A 调整 |
...
F14 基础治理 ──→ F1 中继转发 ─────────────────────────→ 核心基座
    ...
    │               ├── F2 伪装回复 ──→ (依赖 F6 的拦截决策)
    │               │
    │               ├── F3 训练样本/轻量审计 ──→ F8 管理后台
    │               │
    │               └── F15 权限预留 ──→ F16 临时审批 ──→ F17 文件安全
    │
    ├── F7 审计追溯 + F3 训练样本 + F15 APIKey ──→ F21 报表中心
    │
...
| 数据治理 | 训练数据覆盖分析 | 按 `user_id + api_key_id` 分组，用确定性 JSON 前缀比较保留最长有效轨迹集合；不按模型隔离、不使用 lookback、不引入外部通信 |
| 数据治理 | 内网升级数据边界 | 保留 `api_keys` 表，不从 JSON/SQL 重新导入 APIKey；其他当前系统表可删除并按最新模型重建 |
