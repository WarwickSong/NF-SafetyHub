...
| **功能 ID** | F3 |
| **优先级** | P0 — 必须实现 |
| **描述** | Chat 对话的 prompt 和 response 完整记录到当前运行数据库，支持按用户/时间/关键词检索；早期 SQLite 运行库已支持该能力，当前上线运行库已切换 PostgreSQL；阶段 3 同步记录文生图元数据，并异步保存文生图图片本体和归档状态 |
...
| 文生图元数据与图片资产 | ImageGenerationMetadata / ImageAsset | prompt、model、size、style、n、response URL 或 b64 存在状态、request_id、用户、时间；图片资产异步保存本地路径、sha256、mime_type、size_bytes 和下载/解码状态 |
...
| image_metadata | TEXT | 文生图元数据 JSON，记录 prompt、参数、响应引用、资产数量、资产来源和异步归档状态 |
| image_assets | TABLE | 文生图图片资产表，记录 request_id、source_type、source_url、status、local_path、sha256、mime_type、size_bytes、error、created_at、completed_at |
...
| F3-C7 | 阶段 3 文生图记录元数据和响应引用，并对 b64_json 与 URL 图片本体做异步归档；下载/解码失败只写入失败状态，不影响主链路 |
| F3-C8 | 图片本体保存到受控本地目录，记录 sha256、mime_type、size_bytes、local_path、status 和 error；不得直接暴露未鉴权静态 URL |
...
- [x] 文生图请求能记录元数据和响应引用，并异步归档 URL / b64_json 图片本体及状态
