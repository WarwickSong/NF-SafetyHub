...
4. 趋势摘要：日报展示小时曲线，周报展示每日曲线，月报展示每日或自然周曲线。
...
| 系统运行状态 | 新增运行状态采样表，采样来源包括 `/admin/api/runtime` 同源内部方法、健康检查、磁盘空间和 `psutil` | 周期内按采样数据统计平均值、峰值、异常次数和可用空间变化 |
...
| CPU 使用率 | `psutil.cpu_percent()` | 平均值、峰值 |
| 内存使用率 | `psutil.virtual_memory()` | 平均值、峰值 |
...
| `/v1` 并发队列 | `get_v1_concurrency_snapshot()` | 最大在途数、最大排队数、拒绝数增量、超时数增量 |
| 归档队列 | `ArchiveQueue.snapshot()` | 最大队列长度、处理数增量、丢弃数增量 |
| 上游连接池 | 既有上游连接池配置和错误统计 | 配置摘要、上游错误数增量，若当前无错误计数字段则先留空 |

采样频率建议默认 5 分钟一次，可通过配置调整。PDF 报表应优先用曲线展示运行状态变化，日报按小时聚合，周报按日聚合，月报按日或自然周聚合；XLSX 保留曲线数据点和聚合表。采样任务失败不影响 `/v1` 主链路，失败次数进入报表和告警摘要。
