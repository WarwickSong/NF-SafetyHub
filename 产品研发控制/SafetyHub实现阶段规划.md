...
> 本文档将 SafetyHub 的全部功能映射到 10 个产品交付阶段。当前执行口径为：阶段 7 及之后的开发暂不进行，近期只完善阶段 6 及之前能力，确保生产上线稳定；阶段 7~10 内容仅作为长期规划保留，不作为当前上线阻塞项。
...
| 阶段 6 | KeyProvider + 中转站联通 | 当前生产上线范围 | KeyProvider 抽象、中转站联通创建、reveal/复制、Provider-aware 吊销和历史 Key 导入核心能力已完成，当前只做上线稳定性完善 |
...
| S6-10 | ✅ 批量导入支持脚本导入 `yxai_token_export.json`，已导入 16 条历史 Key；CSV/表单粘贴迁移增强待后续 | F15 | P1 |
...
| 历史 Key 导入 | ✅ `scripts/import_yxai_keys.py` 可幂等导入 `yxai_token_export.json`，当前已导入 16 条 |
...
**当前口径**：暂不开发，仅保留规划记录；生产上线前不新增 Prometheus 指标、Webhook 告警、审计导出、图片资产后台预览/下载页面、存储配额和保留策略。
...
**当前生产上线关键路径**：阶段 1 → 阶段 2 → 阶段 3 → 阶段 4 → 阶段 5 → 阶段 6 上线稳定性完善。
...
| v1.1 | 阶段 6 | F15.2 | 当前仅收敛为中转站联通和阶段 6 既有能力生产上线完善；阶段 7~8 暂不开发 |
