...
> 本文档将 SafetyHub 的全部功能映射到 10 个产品交付阶段。当前执行口径为：阶段 7 及之后的开发暂不进行，近期聚焦阶段 6A 单实例 Docker 生产稳定性与高并发治理，确保生产上线稳定；阶段 7~10 内容仅作为长期规划保留，不作为当前上线阻塞项。
...
- [ ] 队列满或排队超时时返回 429/503，不出现请求无限挂起
- [ ] 压测期间 `/health/live`、`/health/ready` 可快速返回
- [ ] 压测期间管理员页面可登录，`/admin/api/stats` 返回缓存结果或可接受延迟结果
- [ ] 非流式和流式 Chat 请求统一使用 `/v1/*` 并发闸门，不单独拆分流式并发池，并保持 OpenAI-compatible 响应格式
- [ ] 上游连接池配置可观测，连接数不随请求数无限增长
- [ ] 归档/审计写入异常或队列满不影响主链路响应
- [ ] Docker 生产启动不包含 `--reload`，容器重启后规则、数据库和 KeyProvider 可正常初始化
...
- [ ] 安全检测耗时 < 10ms/请求
- [ ] 50 并发 SSE 连接稳定
- [ ] `/v1/*` 最大在途请求数受 `V1_MAX_INFLIGHT` 约束，队列满或排队超时能快速返回 429/503
- [ ] 高并发压测期间 `/admin/*` 和 `/health/*` 不被 `/v1/*` 队列阻塞
- [ ] 上游 HTTP 连接池复用生效，连接数不随请求总数无限增长
- [ ] 归档/审计写入削峰或降级不影响主链路返回
