...
> 本文档将 SafetyHub 的全部功能映射到 10 个产品交付阶段。每个阶段都包含明确目标、任务清单、产出物、依赖关系和验收标准。
...
阶段 7   扫描升级（PII 完整 + 关键词扩充 + 规则分级）
阶段 8   可观测性增强 + 告警
阶段 9   临时审批 + Key 级安全策略 + 审批链
阶段 10  远期能力（文件安全 / NER / 配额 / 多上游 / 多租户）
```
...
**目标**：建立数据追溯基础。Chat Completions 请求与响应进入消息归档，文生图请求先做元数据归档但不保存图片本体，所有命中事件进入审计，同时完成 APIKey、配额、策略、审批链的 schema 预留。
...
| OneAPI/OneHub 适配 | 可调用中转站接口创建、吊销、查询 Key |
| OpenAI 兼容适配 | 支持通用兼容协议供应商 |
| 自动续约迁移 | 从旧中转站迁移到新中转站时客户端 Key 不变 |
...
**目标**：从弱扫描升级为完整扫描规则集，覆盖更多 PII 与敏感信息，支持命中分级和规则热加载。
...
**目标**：让管理员能看到运行状态、命中趋势、拦截趋势，并在高风险事件发生时收到通知；同时增强文生图资产能力，实现图片本体异步归档、后台预览/下载、存储配额和保留策略。
...
| S8-07 | 审计日志 CSV 导出，单次上限 10000 条 | F7 | P1 |
| S8-08 | 实现文生图图片本体异步归档：支持 b64_json 解码保存、URL 异步下载、sha256 去重、大小限制和下载状态记录 | F3/F12 | P1 |
| S8-09 | 管理后台增加文生图资产预览/下载能力，下载接口必须鉴权，不直接暴露静态目录 | F8/F12 | P1 |
| S8-10 | 增加图片资产存储配额、保留天数、清理任务和备份恢复演练 | 运维/F20 | P1 |
...
| CSV 导出 | 审计日志可按时间范围导出 |
| 文生图图片资产 | 图片本体异步保存，记录本地路径、sha256、mime_type、size_bytes、下载状态和过期时间 |
| 图片后台访问 | 管理员可在鉴权后台预览/下载图片资产，不暴露未鉴权静态 URL |
...
- [ ] 仪表盘展示今日拦截数、告警数、趋势
- [ ] 指标字段完整，能支撑生产监控
- [ ] 文生图图片本体可异步保存，并记录 sha256、大小、类型和下载状态
- [ ] 管理后台可鉴权预览/下载图片资产，未授权访问被拒绝
- [ ] 图片资产配额、保留天数、清理任务和备份恢复演练通过

---

## 十、阶段 9：临时审批 + Key 级安全策略 + 审批链

**目标**：启用差异化治理能力。不同 APIKey 可绑定不同安全策略、审批链和审批人，高危规则不可降级或审批放行。
...
**目标**：在核心产品稳定后补充增强能力，避免影响阶段 1~9 的关键路径。
...
**并行机会**：阶段 6、7、8 在阶段 5 后可以并行推进；阶段 9 依赖阶段 5 的 APIKey 关联、阶段 7 的规则分级、阶段 8 的告警能力。
...
| 风险 | 概率 | 影响 | 缓解措施 |
|------|------|------|----------|
| SSE 长连接压垮服务器 | 低 | 高 | Nginx 限流 + Uvicorn worker 控制 + 阶段 8 指标监控 + 压力测试 |
| 规则误杀正常对话 | 中 | 中 | 阶段 2 只启用弱扫描；阶段 7 用回归测试和误报率监控再扩充 |
| 规则被绕过 | 高 | 高 | 阶段 7 专门覆盖 URL 编码、分段发送、Unicode 混淆 |
| 数据库性能瓶颈 | 低 | 中 | SQLite 百万条以内够用，阶段 10 预留 PostgreSQL 迁移 |
...
| v1.1 | 阶段 6~8 | F4、F5、F7、F9、F12、F15.2 | 中转站联通、扫描升级、可观测性与告警增强 |
| v1.2 | 阶段 9 | F16、F18、F19 | 审批、安全策略、审批链启用 |
| v2.0+ | 阶段 10 | F10、F11、F13、F17、F20 | 文件安全、NER、配额、多上游、多租户等远期能力 |
