...
class ApiKeyService:
    ...
    async def create(self, payload: ApiKeyCreate) -> ApiKeyCreateResult:
        ...
        async with self._session_factory() as session:
            safetyhub_key = upstream_key if payload.reuse_upstream_key else await self._generate_unique_safetyhub_key(session)
            record = _build_api_key_record(payload, upstream_key, safetyhub_key, self._crypto.encrypt(upstream_key))
            ...
            return ApiKeyCreateResult(record=record, safetyhub_key=None if payload.reuse_upstream_key else safetyhub_key)
    ...
    async def replace_upstream_key(self, api_key_id: str, new_upstream_key: str) -> ApiKeyRecord | None:
        ...
        async with self._session_factory() as session:
            ...
            record.upstream_key_id = key_prefix(raw_key)
...
def _build_api_key_record(payload: ApiKeyCreate, upstream_key: str, safetyhub_key: str, upstream_key_encrypted: str) -> ApiKeyRecord:
    return ApiKeyRecord(
        ...
        upstream_key_id=key_prefix(upstream_key),
